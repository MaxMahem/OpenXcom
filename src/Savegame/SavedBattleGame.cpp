...
void getEnvrionmentShockIndicator(const SavedBattleGame* sbg, const Surface*& shockIndicator)
{
	...
	shockIndicator = enviro && !enviro->getInventoryShockIndicator().empty() ? sbg->getMod()->getSurface(enviro->getInventoryShockIndicator(), false)
																			 : sbg->getMod()->getSurface("BigShockIndicator", false);
}
