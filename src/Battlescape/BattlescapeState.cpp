 ...
 */
void BattlescapeState::drawItem(BattleItem* item, Surface* hand, bool drawReactionIndicator)
{
	hand->clear();
	...
	if (item)
	{
		...
		// get the bounds and set their position to 0, as the item is being drawn relative to a child surface.
		SDL_Rect spriteBounds = item->getInvSpriteBounds();
		SDL_Rect handSlotBounds = SDL_Rect{0, 0, static_cast<Uint16>(hand->getWidth()), static_cast<Uint16>(hand->getHeight())};

		InventoryItemSprite(*item, *hand, spriteBounds).draw(*surfaceSet, *_save, animFrame);
		InventoryItemOverlay(*item, *hand, spriteBounds, InventoryItemOverlay::BATTSCAPE_HAND).draw(*_save, animFrame);
		InventoryItemOverlay(*item, *hand, handSlotBounds, InventoryItemOverlay::BATTSCAPE_HAND).drawHandOverlay(*_save, animFrame);
	}
